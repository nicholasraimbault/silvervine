...
1. Should the opt-in error reporting endpoint be self-hosted (Nick controls) or third-party (e.g., Plausible custom events)? Self-hosted gives full control but requires infra; third-party is zero-effort but introduces dependency.
2. Should V1 ship `neon test` (EME health-check via headless browser) or defer to V1.1? Headless EME testing is finicky and may require maintaining a test page or relying on a third-party (Shaka demo).
3. Should the V2 rewrite happen in-place on `master` (replacing existing code with one large PR) or in a long-lived branch (`v2-rust-rewrite`) with the existing bash code preserved on master until V2 is ready?
4. The repository name remains `neon`. Confirm — or is renaming a consideration (e.g., to clarify it's a DRM patcher)?

---

*This is a design spec, not an implementation plan. Once approved, the next step is to invoke the writing-plans skill to produce a step-by-step implementation plan that breaks V1 into independently-shippable milestones.*
