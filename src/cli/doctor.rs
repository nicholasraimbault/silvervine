//! `neon doctor` — diagnostics + EME error-code translation.
//!
//! ## Modes
//!
//! 1. **Plain** (`neon doctor`) — print a friendly diagnostic summary.
//! 2. **JSON** (`neon doctor --json`) — same data, machine-readable.
//! 3. **Share** (`neon doctor --share`) — print a clickable URL that
//!    pre-fills a GitHub issue with the diagnostic body.
//! 4. **Code translation** (`neon doctor <code>`) — translate an EME
...
pub struct Diagnostics {
    /// `neon` version (from `Cargo.toml`).
    pub neon_version: String,
...
pub fn build_diagnostics(
    ...
    let browsers_snapshot = detected
        ...
        .collect();
    Diagnostics {
        neon_version: env!("CARGO_PKG_VERSION").to_string(),
        heartbeat_at,
        heartbeat_stale,
        current_cdm_version,
...
pub fn render_text(d: &Diagnostics, out: &mut dyn Write) -> std::io::Result<()> {
    writeln!(out, "Neon doctor v{}", d.neon_version)?;
    writeln!(out)?;
    if d.legacy_install_present {
        writeln!(
            out,
            "WARN: A legacy V1 install was detected. Run `neon repair` or"
        )?;
...
///
/// ```text
/// https://github.com/nicholasraimbault/neon/issues/new?template=bug.yml&body=<encoded>
/// ```
#[must_use]
pub fn share_url(diagnostics: &Diagnostics) -> String {
    ...
    let body = format!("```\n{text}\n```\n\n_Generated by `neon doctor --share`._");
    let encoded = urlencoding::encode(&body);
    format!("https://github.com/nicholasraimbault/neon/issues/new?template=bug.yml&body={encoded}")
}
    ...
    if let Some(d) = eme::translate_error_code(code) {
        {
            if output.json {
                ...
                if let Some(cmd) = d.suggested_command {
                    ...
                    writeln!(out, "(neon cannot fix this code automatically)")
                        .map_err(Error::from)?;
                }
            }
        }
        ...
        writeln!(
            out,
            "Unknown error code '{code}'. Try `neon doctor` to check Widevine state."
        )
        .map_err(Error::from)?;
...
mod tests {
    ...
    fn run_translate_no_command_code_says_so() {
        // Netflix M7111-1331-2206 is a VPN block — neon can't fix it.
        let mut buf = Vec::new();
        let opts = OutputOptions::default();
