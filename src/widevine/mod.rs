...
//! and [`cache`]. V3-Phase A scaffolding adds [`provider`] — the
//! `CdmProvider` trait abstraction that lets the patch flow consume CDM
//! bytes from any source (V2: local file cache; V3: localhost-bridge
//! VM).
//!
...
pub use cache::{
    current as current_cdm, current_provider, current_provider_in, default_cache_root,
    ensure_cdm_for, prune as prune_cache, rollback as rollback_cdm, verify_integrity, CachedCdm,
};
...
pub use provider::{CdmProvider, LocalFileCdm};
